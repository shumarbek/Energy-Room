document.addEventListener("DOMContentLoaded", () => {
    ...
    function filterProducts(category) {
        ...
        const productsFilters = document.querySelector('#productsFilters');
        productsFilters.classList.add('left-[-500%]');
        productsFilters.style.boxShadow = "none";
...
function openProductsFilters() {
    if (productsFilters.classList.contains('left-[-500%]')) {
        productsFilters.classList.remove('left-[-500%]');
        productsFilters.classList.add('left-0');
        productsFilters.style.transition = "right 0.3s ease-in-out";
        productsFilters.style.boxShadow = "0 10px 10px 1000px rgba(0, 0, 0, 0.5)";
    } else {
...
document.addEventListener('click', (e) => {
    if (!e.target.closest('#categoryNavigation')) {
        const productsFilters = document.querySelector('#productsFilters');
        productsFilters.classList.remove('left-0');
        productsFilters.classList.add('left-[-500%]');
        productsFilters.style.boxShadow = "none";
